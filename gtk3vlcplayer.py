...
class ApplicationWindow(Gtk.Window):
    ...
    def on_audio_track(self, data):
        dialog = DialogAudioSub(self, self.audio_tracks_list, data, "Choose a different audio track:")
        response = dialog.run()
    ...
    def on_sub_track(self, data):
        dialog = DialogAudioSub(self, self.subs_tracks_list, data, "Choose a different subtitle track:")
        response = dialog.run()
        if response == Gtk.ResponseType.OK:
    ...
    def __init__(self, parent, track_list, data, label):
        ...
        for item in self.track_list:
            if self.data in item:
                self.combo.set_active(self.track_list.index(item))
